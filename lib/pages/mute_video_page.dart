...
class _MuteVideoPageState extends State<MuteVideoPage> {
  ...
  Future<void> _mute() async {
    ...
    try {
      ...
      if (ReturnCode.isSuccess(rc)) {
        ...
        setState(() {});
        await newCtrl.play();
      } else { _showError(l10n.muteFailed); }
    } catch (e) { _showError(l10n.muteError(e.toString())); }
  ...
  Widget _buildAppBar(AppLocalizations l10n) => Container(padding: const EdgeInsets.symmetric(horizontal:8,vertical:8), child: Row(children: [
    ...
    Expanded(child: Text(l10n.muteVideo, style: const TextStyle(color:Colors.white,fontSize:20,fontWeight:FontWeight.bold), textAlign:TextAlign.center, maxLines:1, overflow:TextOverflow.ellipsis)),
  ...
  Widget _buildWorkArea(AppLocalizations l10n) => SingleChildScrollView(padding: const EdgeInsets.all(16), child: Column(children: [
    ...
    Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color:Colors.white, borderRadius:BorderRadius.circular(16)), child: Row(children: [
      const Icon(Icons.volume_off, color: Colors.purple), const SizedBox(width:12), Expanded(child: Text(l10n.muteDescription, style: const TextStyle(fontSize:14))),
    ])),
    ...
    if (_resultPath == null) SizedBox(width:double.infinity,height:52, child: ElevatedButton(onPressed: _isProcessing?null:_mute,
      ...
      child: _isProcessing ? Row(mainAxisAlignment:MainAxisAlignment.center,children:[SizedBox(width:24,height:24,child:CircularProgressIndicator(color:Colors.white,strokeWidth:2.5)),SizedBox(width:12),Text(l10n.muting,style:TextStyle(fontSize:16,fontWeight:FontWeight.w600))]) : Row(mainAxisAlignment:MainAxisAlignment.center,children:[const Icon(Icons.volume_off),const SizedBox(width:10),Flexible(child: Text(l10n.removeAudio,style:const TextStyle(fontSize:17,fontWeight:FontWeight.w600),overflow:TextOverflow.ellipsis))]),
    )) else Row(children: [
